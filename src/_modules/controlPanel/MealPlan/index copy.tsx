...
const MealPlans = () => {
    ...
    // Auto-consume past meals
    const autoConsumePastMeals = async () => {
        try {
            const response = await fetch('/api/meal-plans/auto-consume', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                if (data.updatedCount > 0) {
                    toast({
                        title: "Auto-consumed meals",
                        description: `${data.updatedCount} past meals automatically marked as consumed`,
                        status: "info",
                        duration: 3000,
                        isClosable: true,
                    });
            ...
            console.error('Error auto-consuming past meals:', error);
        }
    };
    ...
    useEffect(() => {
        if (session?.user?.id) {
            // Auto-consume past meals first, then fetch data
            autoConsumePastMeals().then(() => {
                fetchMealPlans();
    ...
    // Set up interval to check for past meals every hour
    useEffect(() => {
        if (session?.user?.id) {
                                        ...
                                        {plannedMeals.length === 0 ? (
                                            ...
                                            plannedMeals.map((meal) => (
                                                <Tr key={meal.id}>
                                                    ...
                                                    <Td>
                                                        <Badge
                                                            colorScheme={isDatePassed(meal.date) ? "orange" : "blue"}
                                                        >
                                                            {isDatePassed(meal.date) ? "Overdue" : "Planned"}
                                                        </Badge>
                                                    </Td>
                                                    <Td>
                                                        <Button
